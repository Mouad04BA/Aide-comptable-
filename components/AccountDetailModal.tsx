...
import { ChartAccount, AccountExplanationResponse } from '../types';
...
interface AccountDetailModalProps {
  ...
  explanation: AccountExplanationResponse | null;
  isLoading: boolean;
  error: string | null;
}
      ...
      >
        <div className="sticky top-0 bg-white dark:bg-dark-card z-10 px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
          <div>
            <h2 id="modal-title" className="text-xl font-bold text-gray-900 dark:text-white">
              {t('accountDetailTitle')}
            </h2>
            <p className="text-sm text-gray-500 dark:text-gray-400 font-mono mt-1">{account.code} - {account.name[language]}</p>
          </div>
          <button
            onClick={onClose}
            className="p-2 rounded-full text-gray-500 hover:bg-gray-200 dark:hover:bg-gray-700 dark:text-gray-300"
        ...
        <div className="p-6">
          {isLoading && (
            <div>
              <LoadingSpinner />
              <p className="text-center text-gray-500 dark:text-gray-400">{t('loadingExplanation')}</p>
            </div>
          )}
          ...
          {explanation && !isLoading && (
            <div className="space-y-6">
              ...
              <div>
                <h3 className="text-lg font-semibold mb-2 text-secondary-dark">{t('exampleSectionTitle')}</h3>
                <p className="text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-dark-bg p-4 rounded-md whitespace-pre-wrap">{explanation.example}</p>
              </div>
            </div>
          )}
        </div>
